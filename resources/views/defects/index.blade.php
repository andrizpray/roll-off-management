...
<div class="animate-in space-y-4">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div class="glass stat-card purple p-4">
            <div class="flex items-center justify-between mb-2">
        ...
        <div class="glass stat-card blue p-4">
            <div class="flex items-center justify-between mb-2">
                <span class="text-[10px] font-semibold uppercase tracking-wide" style="color: #60a5fa;">Ditampilkan</span>
                <div class="w-7 h-7 rounded-lg flex items-center justify-center" style="background: rgba(59,130,246,0.12);">
                    <i class="fas fa-filter text-xs" style="color: #60a5fa;"></i>
                </div>
            </div>
    ...
    <div class="glass overflow-hidden">
        <button onclick="document.getElementById('defectFilters').classList.toggle('hidden')"
            ...
            <i class="fas fa-chevron-down text-xs transition-transform" style="color: #484f58;"></i>
        </button>
                ...
                <form method="GET" action="{{ route('defects.index') }}" id="filterForm">
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                        <div class="col-span-2 md:col-span-1">
                            <label class="block text-[10px] font-semibold uppercase tracking-wide mb-1.5" style="color: #484f58;">Search</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-semibold uppercase tracking-wide mb-1.5" style="color: #484f58;">Tahun</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-semibold uppercase tracking-wide mb-1.5" style="color: #484f58;">Alasan</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-semibold uppercase tracking-wide mb-1.5" style="color: #484f58;">Bulan</label>
                            <div class="flex gap-1.5">
        ...
        <div class="overflow-x-auto">
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="color: #30363d;">{{ ($defects->currentPage()-1)*$defects->perPage()+$i+1 }}</td>
                        <td><span class="tag tag-gray">{{ $def->year }}</span></td>
                        <td>
                            @if($def->lot_id)
                                <a href="{{ route('items.index', ['search' => $def->lot_id]) }}" class="font-semibold no-underline hover:underline" style="color: #60a5fa;">{{ $def->lot_id }}</a>
                            @else
                    ...
                    <tr>
                        <td colspan="10" class="text-center py-10">
                            ...
                            <span style="color: #484f58;">Tidak ada defect</span>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="md:hidden">
        ...
        <div class="mobile-card" @if($def->lot_id) onclick="window.location='{{ route('items.index', ['search' => $def->lot_id]) }}'" style="cursor: pointer;" @endif>
            <div class="flex items-center justify-between mb-2">
                ...
                @else
                    <span class="text-xs" style="color: #484f58;">No Lot ID</span>
                @endif
            ...
            <div class="flex justify-between text-xs mb-1.5">
                <span style="color: #484f58;">Paper</span>
                <span style="color: #c9d1d9;">{{ $def->paper_type ?? '-' }} / {{ $def->gsm ?? '-' }} GSM</span>
            </div>
